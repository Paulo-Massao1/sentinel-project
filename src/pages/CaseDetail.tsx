...
import type { Observation } from "../hooks/useCases";
...
export default function CaseDetail() {
  ...
  async function handleExportPdf() {
    ...
    // Child info from first observation that has it
    const firstWithChildInfo = chronologicalObservations.find(
      (o) => o.childInfo,
    );
    if (firstWithChildInfo) {
      yPos = addInlineField(
        t("document.pdf.childInfo") + ":",
        firstWithChildInfo.childInfo,
        margin,
        yPos,
        contentWidth,
        true,
    ...
    // Helper to render or measure an observation box
    function renderObservationBox(
      ...
      return y + 2; // box bottom padding
    }

    // Observations (chronological for narrative)
    for (let i = 0; i < chronologicalObservations.length; i++) {
      const obs = chronologicalObservations[i]!;

      // Measure height without drawing
      ...
      // Draw box background then content on top
